...
// Fungsi untuk mendapatkan data produk populer dari semua kategori
function getPopularProducts($db) {
    ...
    $sqlMac = "
        SELECT 
            ...
            'mac' as category,
            MIN(k.harga) as price,
            g.foto_thumbnail as image
        ...
        GROUP BY p.id, p.nama_produk, g.foto_thumbnail
        ORDER BY p.id DESC
        LIMIT 4
    ";
    $stmtMac = $db->prepare($sqlMac);
    $stmtMac->execute();
    $productsMac = $stmtMac->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($productsMac as &$product) {
        $product['price'] = 'Rp ' . number_format($product['price'], 0, ',', '.');
        $product['image'] = !empty($product['image']) ? '../../admin/uploads/' . $product['image'] : '';
        $product['rating'] = 4.5; // Default rating
        $product['badge'] = ['text' => 'Populer', 'type' => 'hot'];
    }
    $allProducts = array_merge($allProducts, $productsMac);
    ...
    $sqlIphone = "
        ...
        LIMIT 4
    ";
    $stmtIphone = $db->prepare($sqlIphone);
    $stmtIphone->execute();
    $productsIphone = $stmtIphone->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($productsIphone as &$product) {
        $product['price'] = 'Rp ' . number_format($product['price'], 0, ',', '.');
        $product['image'] = !empty($product['image']) ? '../../admin/uploads/' . $product['image'] : '';
        $product['rating'] = 4.7;
        $product['badge'] = ['text' => 'Terlaris', 'type' => 'hot'];
    }
    $allProducts = array_merge($allProducts, $productsIphone);
    ...
    $sqlIpad = "
        ...
        LIMIT 4
    ";
    $stmtIpad = $db->prepare($sqlIpad);
    $stmtIpad->execute();
    $productsIpad = $stmtIpad->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($productsIpad as &$product) {
        $product['price'] = 'Rp ' . number_format($product['price'], 0, ',', '.');
        $product['image'] = !empty($product['image']) ? '../../admin/uploads/' . $product['image'] : '';
        $product['rating'] = 4.6;
        $product['badge'] = ['text' => 'Populer', 'type' => 'hot'];
    }
    $allProducts = array_merge($allProducts, $productsIpad);
    ...
    $sqlWatch = "
        ...
        LIMIT 4
    ";
    $stmtWatch = $db->prepare($sqlWatch);
    $stmtWatch->execute();
    $productsWatch = $stmtWatch->fetchAll(PDO::FETCH_ASSOC);
    ...
    $allProducts = array_merge($allProducts, $productsWatch);
    ...
    $sqlAksesori = "
        ...
        LIMIT 4
    ";
    $stmtAksesori = $db->prepare($sqlAksesori);
    $stmtAksesori->execute();
    $productsAksesori = $stmtAksesori->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($productsAksesori as &$product) {
        $product['price'] = 'Rp ' . number_format($product['price'], 0, ',', '.');
        $product['image'] = !empty($product['image']) ? '../../admin/uploads/' . $product['image'] : '';
        $product['rating'] = 4.3;
        $product['badge'] = ['text' => 'Aksesori', 'type' => 'hot'];
    }
    $allProducts = array_merge($allProducts, $productsAksesori);
    ...
    $stmtMusic->execute();
    $productsMusic = $stmtMusic->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($productsMusic as &$product) {
        $product['price'] = 'Rp ' . number_format($product['price'], 0, ',', '.');
        $product['image'] = !empty($product['image']) ? '../../admin/uploads/' . $product['image'] : '';
        $product['rating'] = 4.5;
        $product['badge'] = ['text' => 'Audio', 'type' => 'hot'];
    }
    $allProducts = array_merge($allProducts, $productsMusic);
    ...
    $sqlAirtag = "
        ...
        LIMIT 4
    ";
    $stmtAirtag = $db->prepare($sqlAirtag);
    $stmtAirtag->execute();
    $productsAirtag = $stmtAirtag->fetchAll(PDO::FETCH_ASSOC);
    
    foreach ($productsAirtag as &$product) {
        $product['price'] = 'Rp ' . number_format($product['price'], 0, ',', '.');
        $product['image'] = !empty($product['image']) ? '../../admin/uploads/' . $product['image'] : '';
        $product['rating'] = 4.2;
        $product['badge'] = ['text' => 'Tracker', 'type' => 'hot'];
    }
    $allProducts = array_merge($allProducts, $productsAirtag);
    
    return $allProducts;
}

// Fungsi untuk mendapatkan produk terbaru dari semua kategori
function getLatestProducts($db) {
    $latestProducts = [];
    
    // Ambil dari semua kategori, urutkan berdasarkan created_at terbaru
    $allProducts = [];
    ...
    $sqlMac = "
        ...
        LEFT JOIN admin_produk_mac_kombinasi k ON p.id = k.produk_id
        WHERE g.foto_thumbnail IS NOT NULL
        GROUP BY p.id, p.nama_produk, g.foto_thumbnail, p.created_at
    ...
    foreach ($latestProducts as &$product) {
        $product['price'] = 'Rp ' . number_format($product['price'], 0, ',', '.');
        $product['image'] = !empty($product['image']) ? '../../admin/uploads/' . $product['image'] : '';
        $product['rating'] = 4.8; // Default rating untuk produk baru
        $product['badge'] = ['text' => 'Baru', 'type' => 'new'];
    }
...
$popularProducts = getPopularProducts($db);
$latestProducts = getLatestProducts($db);
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Data produk dari PHP (JSON yang dikonversi dari array PHP)
            const popularProducts = <?php echo $popularProductsJSON; ?>;
            const latestProducts = <?php echo $latestProductsJSON; ?>;
            ...
            function renderProductsSlider(products, containerId, title = '') {
                if (!products || products.length === 0) {
                    ...
                    if (container) {
                        container.innerHTML = '<p style="text-align: center; padding: 40px; color: #666;">Belum ada produk yang tersedia.</p>';
                    }
                    return;
                }
            ...
            function createProductCard(product) {
                // Handle gambar - jika tidak ada, tampilkan placeholder
                const imageHTML = product.image ? 
                    `<img src="${product.image}" alt="${product.name}" class="product-image" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'no-image\\'><i class=\\'bi bi-image\\'></i><br/>No Image</div>';" />` :
                    `<div class="no-image"><i class="bi bi-image"></i><br/>No Image</div>`;
            ...
            document.addEventListener('click', function(e) {
                if (e.target.classList.contains('product-btn') || e.target.closest('.product-btn')) {
                    const productBtn = e.target.classList.contains('product-btn') ? 
                        e.target : e.target.closest('.product-btn');
