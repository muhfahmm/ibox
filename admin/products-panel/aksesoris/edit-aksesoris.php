...
foreach($combinations as $c) {
    if (!isset($types_map[$c['tipe']])) {
        $types_map[$c['tipe']] = true;
    }
    if (!isset($sizes_map[$c['ukuran']])) {
        $sizes_map[$c['ukuran']] = true;
    }
    
    // Create a key for stock lookup
    $key = $c['warna'] . '|' . $c['tipe'] . '|' . $c['ukuran'];
    $stocks_map[$key] = [
        'jumlah_stok' => $c['jumlah_stok'],
        'harga' => $c['harga'],
...
// Kategori options
$kategori_options = ['case', 'charger', 'headphone', 'keyboard', 'mouse', 'trackpad', 'adapter', 'other'];
?>
...
<head>
    ...
    <style>
        ...
        .color-option, .type-option, .size-option, .kompatibel-option {
            background: white;
            padding: 15px;
            border-radius: 8px;
...
<body>
    ...
    <script>
                ...
                <div class="form-row">
                    <div class="form-col col-3">
                        ...
                        <input type="text" class="form-control" name="warna[${newIndex}][nama]" 
                               placeholder="Nama Warna (Contoh: Hitam)" required value="${data ? data.nama : ''}" onchange="generateCombinations()">
                    </div>
        ...
        function addType(data = null) {
            ...
            newType.innerHTML = `
                <input type="text" class="form-control" name="tipe[${newIndex}]" 
                       placeholder="Tipe (Contoh: Silicone, Leather)" required value="${data ? data : ''}" onchange="generateCombinations()">
        ...
        function addSize(data = null) {
            ...
            newSize.innerHTML = `
                <input type="text" class="form-control" name="ukuran[${newIndex}]" 
                       placeholder="Ukuran (Contoh: Standard, Large) - Opsional" value="${data ? data : ''}" onchange="generateCombinations()">
        ...
        function generateCombinations() {
            ...
            document.querySelectorAll('.size-option input').forEach(input => {
                const val = input.value.trim();
                // For sizes, empty is allowed
                sizes.push(val);
            });
            
            // Generate table
            const totalCombinations = colors.length * types.length * Math.max(sizes.length, 1);
            const sizesCount = sizes.filter(s => s).length || 1;
            ...
            if (totalCombinations > 0) {
                ...
                colors.forEach((color, cIdx) => {
                    types.forEach((type, tIdx) => {
                        // If no sizes specified, use empty string
                        const sizeList = sizes.filter(s => s).length > 0 ? sizes.filter(s => s) : [''];
                        ...
                        sizeList.forEach((size, sIdx) => {
                            ...
                            const existingData = initialData.stocks[key] || {};
        ...
        document.addEventListener('input', function(e) {
            if (e.target.matches('input[name*="[nama]"], input[name*="tipe"], input[name*="ukuran"]')) {
