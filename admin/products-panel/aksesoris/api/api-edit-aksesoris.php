...
try {
    ...
    if (empty($_POST['kategori'])) throw new Exception("Kategori harus diisi");
    ...
    foreach ($warna_data as $color_index => $color_info) {
        $warna = mysqli_real_escape_string($db, $color_info['nama'] ?? '');
        if (empty($warna)) continue;
        ...
        if (!$thumbnail_name) {
            throw new Exception("Thumbnail untuk warna $warna harus ada");
        }
        ...
        $query_gambar = "INSERT INTO admin_produk_aksesoris_gambar 
                        ...
                        VALUES ('$product_id', '$warna', '$thumbnail_name', '$product_images_json')";
        
        if (!mysqli_query($db, $query_gambar)) {
            throw new Exception("Gagal menyimpan gambar warna $warna: " . mysqli_error($db));
        }
    }
    ...
    error_log("Edit Aksesoris Error: " . $e->getMessage());
}
