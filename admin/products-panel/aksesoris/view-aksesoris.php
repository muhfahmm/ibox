...
$query_product = "SELECT * FROM admin_produk_aksesoris WHERE id = '$product_id'";
$result_product = mysqli_query($db, $query_product);

if (mysqli_num_rows($result_product) === 0) {
    header('Location: aksesoris.php?error=product_not_found');
    exit();
}

$product = mysqli_fetch_assoc($result_product);

// Decode kompatibel_dengan
$kompatibel_data = json_decode($product['kompatibel_dengan'], true) ?? [];

// Fetch all combinations (variants)
$query_combinations = "SELECT * FROM admin_produk_aksesoris_kombinasi WHERE produk_id = '$product_id' ORDER BY warna, tipe, ukuran";
...
foreach ($combinations as $combo) {
    ...
    if (!in_array($combo['tipe'], $types)) {
        $types[] = $combo['tipe'];
    }
    if (!in_array($combo['ukuran'], $sizes) && !empty($combo['ukuran'])) {
        $sizes[] = $combo['ukuran'];
    }
...
<head>
    ...
    <title>Detail Produk Aksesoris - <?php echo htmlspecialchars($product['nama_produk']); ?></title>
    ...
    <div class="container">
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <h2>
                        ...
                        Detail Produk Aksesoris
                    </h2>
                    <div>
                        <a href="aksesoris.php" class="btn-back me-2">
                            <i class="fas fa-arrow-left"></i> Kembali
                        </a>
                        <a href="edit-aksesoris.php?id=<?php echo $product_id; ?>" class="btn-edit">
            ...
            <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <div class="product-meta">
                                <span class="kategori-badge"><?php echo $kategori_labels[$product['kategori']] ?? ucfirst($product['kategori']); ?></span>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Kompatibel Dengan -->
                <?php if(!empty($kompatibel_data)): ?>
                <div class="mb-5">
                    <h4 class="section-title">Kompatibel Dengan</h4>
                    <div class="p-3 bg-white rounded border">
                        <?php foreach($kompatibel_data as $device): ?>
                            <span class="kompatibel-tag"><?php echo htmlspecialchars($device); ?></span>
                        <?php endforeach; ?>
                    </div>
                </div>
                <?php endif; ?>
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="p-3 bg-white rounded border">
                            ...
                            <?php foreach($types as $type): ?>
                                <span class="badge-custom"><?php echo htmlspecialchars($type); ?></span>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div class="p-3 bg-white rounded border">
                            <h6 class="text-muted mb-2">Ukuran</h6>
                            <?php if(!empty($sizes)): ?>
                                <?php foreach($sizes as $size): ?>
                                    <span class="badge-custom"><?php echo htmlspecialchars($size); ?></span>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <span class="text-muted"><i>Tidak ada ukuran spesifik</i></span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                <?php if(count($combinations) > 0): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($combo['tipe']); ?></td>
                                        <td>
                                            <?php if(!empty($combo['ukuran'])): ?>
                                                <span class="badge bg-info bg-opacity-10 text-info">
                                                    <?php echo htmlspecialchars($combo['ukuran']); ?>
                                                </span>
                                            <?php else: ?>
                ...
                <div class="d-flex justify-content-between mt-5 pt-4 border-top">
                    ...
                    <div>
                        <a href="edit-aksesoris.php?id=<?php echo $product_id; ?>" class="btn-edit me-2">
                            <i class="fas fa-edit"></i> Edit Produk
                        </a>
                        <a href="delete-aksesoris.php?id=<?php echo $product_id; ?>" 
