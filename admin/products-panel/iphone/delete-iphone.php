...
$delete_combinations = "DELETE FROM admin_produk_iphone_kombinasi WHERE produk_id = '$product_id'";
...
if (mysqli_query($db, $delete_product)) {
    header('Location: iphone.php?success=product_deleted');
} else {
    // Jika gagal, log error
    error_log("Gagal menghapus produk iPhone ID $product_id: " . mysqli_error($db));
    header('Location: iphone.php?error=delete_failed');
}
exit();
?>
