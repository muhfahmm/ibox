...
try {
    ...
    if (empty($_POST['warna']) || empty($_POST['tipe']) || empty($_POST['konektivitas'])) {
        throw new Exception("Minimal satu warna, tipe, dan konektivitas harus diisi");
    }
    ...
    // Proses data tipe
    $tipe_data = $_POST['tipe'] ?? [];
    $tipe_list = [];
    
    foreach ($tipe_data as $tp) {
        $tp_clean = mysqli_real_escape_string($db, $tp);
        if (!empty($tp_clean)) {
            $tipe_list[] = $tp_clean;
        }
    }
    ...
    $response['message'] = "Produk Music berhasil ditambahkan dengan $combination_count kombinasi";
