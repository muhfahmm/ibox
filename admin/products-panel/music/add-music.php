...
<head>
    ...
    <title>Tambah Produk Music</title>
...
<body>
    ...
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <a href="music.php" class="btn btn-outline-secondary">
                <i class="fas fa-arrow-left me-2"></i> Kembali
            </a>
            <div class="text-muted">
                Admin Panel / Music / Tambah Produk
            </div>
        </div>
        ...
        <div class="card">
            ...
            <form id="addProductForm" enctype="multipart/form-data">
                <div class="card-body">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="nama_produk" name="nama_produk" placeholder="Contoh: AirPods Pro 2" required>
                            </div>
                    ...
                    <!-- Pilihan Tipe -->
                    <div class="section-title">
                        <i class="fas fa-headphones me-2"></i> Pilihan Tipe
                    </div>
                    
                    <div id="tipe-container">
                        <!-- Default Tipe Added by JS -->
                    </div>
                    
                    <button type="button" class="btn-add-option" onclick="addTipeOption()">
                    ...
                    <div id="konektivitas-container">
                        <!-- Default Konektivitas Added by JS -->
                    </div>
                    
                    <button type="button" class="btn-add-option" onclick="addKonektivitasOption()">
                    ...
                    <div class="d-grid gap-2 mt-5">
                        <button type="submit" class="btn btn-primary btn-lg">
                            <i class="fas fa-save me-2"></i> Simpan Produk Music
                        </button>
                    </div>
    ...
    <script>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control color-name" name="warna[${colorIndex}][nama]" placeholder="Contoh: White" required onkeyup="updateCombinations()">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
        ...
        // --- Add Tipe ---
        function addTipeOption() {
            const container = document.getElementById('tipe-container');
            const html = `
                <div class="option-card tipe-option position-relative">
                    <button type="button" class="btn-remove" onclick="removeOption(this, 'tipe')"><i class="fas fa-times"></i></button>
                    <div class="row align-items-center">
                        <div class="col-md-11">
                            <input type="text" class="form-control tipe-value" name="tipe[]" placeholder="Contoh: Pro, Max, Generation 3" required onkeyup="updateCombinations()">
                        </div>
                    </div>
                </div>`;
            container.insertAdjacentHTML('beforeend', html);
        ...
        // --- Add Konektivitas ---
        function addKonektivitasOption() {
            const container = document.getElementById('konektivitas-container');
            const html = `
        ...
        function generateCombinations() {
            ...
            if (colors.length === 0 || tipes.length === 0 || konektivitases.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted py-4">Data belum lengkap</td></tr>';
                return;
            }
        ...
        document.getElementById('addProductForm').addEventListener('submit', function(e) {
            ...
            if (!kategori) {
                showFeedback('Kategori harus dipilih', 'error');
                return;
            }
            
            // Validate at least one color, tipe, and konektivitas
            const colorInputs = Array.from(document.querySelectorAll('.color-name')).filter(i => i.value.trim());
            const tipeInputs = Array.from(document.querySelectorAll('.tipe-value')).filter(i => i.value.trim());
            const konektivitasInputs = Array.from(document.querySelectorAll('.konektivitas-value')).filter(i => i.value.trim());
            
            if (colorInputs.length === 0) {
                showFeedback('Minimal satu warna harus diisi', 'error');
                return;
            }
            
            if (tipeInputs.length === 0) {
                showFeedback('Minimal satu tipe harus diisi', 'error');
                return;
            }
            
            if (konektivitasInputs.length === 0) {
                showFeedback('Minimal satu konektivitas harus diisi', 'error');
                return;
            }
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    setTimeout(() => window.location.href = 'music.php', 1500);
                } else {
