...
$delete_product = "DELETE FROM admin_produk_music WHERE id = '$product_id'";

if (mysqli_query($db, $delete_product)) {
    header('Location: music.php?success=product_deleted');
} else {
    // Jika gagal, log error
    error_log("Gagal menghapus produk Music ID $product_id: " . mysqli_error($db));
    header('Location: music.php?error=delete_failed');
}
exit();
?>